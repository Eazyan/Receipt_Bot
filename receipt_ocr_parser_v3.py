...
def variants_from_image(img: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    den_clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8)).apply(denoise)

    sharp_blur = cv2.GaussianBlur(gray, (0, 0), 3)
    sharp = cv2.addWeighted(gray, 1.7, sharp_blur, -0.7, 0)
    ...
    th_adapt = cv2.adaptiveThreshold(den_clahe, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 11)
...
def make_ocr(lang: str = "ru") -> PaddleOCR:
    init_kwargs = {
        "lang": lang,
        # PaddleOCR 3.x prefers this.
        "use_textline_orientation": True,
        # PaddleOCR 2.x fallback.
        "use_angle_cls": True,
        # These help if supported; harmlessly filtered out if not.
...
def call_ocr_compat(ocr: PaddleOCR, img: np.ndarray) -> Any:
    ...
    for kwargs in attempts:
        try:
            filtered = filter_kwargs_for_callable(ocr.ocr, kwargs)
            if filtered:
                return ocr.ocr(**filtered)
        except TypeError:
            pass
        except Exception:
            raise
